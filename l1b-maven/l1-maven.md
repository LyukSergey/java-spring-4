...
Це **каталог**, за якою бібліотекар (Maven) знаходить книгу яка вам потрібна.
...
`scope` визначає, на якому потрібна та чи інша залежність. Це критично важливо для оптимізації та уникнення конфліктів.
...
public class MyService {
    public void connectToDatabase(String url, String user, String pass) {
        // Використовуємо стандартний "пульт" від Java
        Connection connection = DriverManager.getConnection(url, user, pass);
        // ... робимо щось з 'connection'
    }
}
```
DriverManager.getConnection("jdbc:postgresql://..."), DriverManager діє як посередник. 
Він сканує всі доступні бібліотеки (JAR-файли) на вашому шляху класів (classpath) і шукає ті, що можуть обробити URL, 
який починається з jdbc:postgresql.
Саме тут і потрібен наш драйвер **postgresql.jar**. 
Усередині нього є спеціальний файл, який "реєструє" його в системі DriverManager. 
DriverManager знаходить його і Делегую завдання йому.

* `import`: Спеціальний scope для імпорту **BOM (Bill of Materials)**.

-----

### **5: Життєвий цикл, Фази, Плагіни та Цілі (Goals)**
...
* **`process-resources`**:
    ...
    **Ціль `resources:resources`** плагіна `maven-resources-plugin` копіює всі файли з директорії 
    `src/main/resources` до `target/classes`. 
     На цьому етапі може відбуватися фільтрація (наприклад, заміна `${project.version}` у файлі `application.properties/yaml` 
     на реальну версію).
