...
Його мета — надати дозвіл, не передаючи пароль.

* **OIDC (OpenID Connect):** Це шар **аутентифікації**, побудований поверх OAuth2. 
Він додає до OAuth2 **ID Token** — спеціальний JWT, який підтверджує особу користувача.

## OAuth2: Ви даєте дозвіл

Приклад:
Ви використовуєте новий фоторедактор онлайн (назвемо його SuperFoto) 
і хочете відредагувати фотографії зі свого Google Photos.
SuperFoto каже: "Мені потрібен доступ до ваших Google Photos, щоб їх показати".
Ви натискаєте "Дозволити".
Вас перенаправляє на сторінку Google, де вас запитують: 
"Дозволити додатку SuperFoto переглядати ваші фотографії?".
Ви натискаєте "Дозволити".
У цей момент ви виконали дію OAuth2. 
Ви не відкриваєте SuperFoto, хто ви, ви лише дали йому дозвіл "читати фото".

## OIDC: як приклад логін через Google
https://accounts.google.com/o/oauth2/v2/auth?
...
&scope=openid%20email%20profile
...

OIDC (OpenID Connect) — це процес входу на сайт під своїм іменем, використовуючи вже існуючий акаунт. 
Це шар аутентифікації, побудований поверх OAuth2.

Приклад:
Ви заходите на сайт для продажу квитків (Tickets.ua) і бачите кнопку "Увійти через Google".
Ви натискаєте цю кнопку.
Вас перенаправляє на сторінку входу Google.
Ви вводите свій логін і пароль від Google.
Вас повертає на сайт Tickets.ua, і ви бачите напис: "Вітаємо, [Ваше ім'я]!".
У цей момент ви виконали дію OIDC. 
Ви не просто дали дозвіл, ви дозволили Google підтвердити вашу особу для сайту Tickets.ua. 
Додаток отримав не тільки доступ, але й ваш "цифровий паспорт" (ID Token), щоб знати, що це саме ви
...
### \#\# Client authentication: OFF. Що таке `Client Secret` і де він має жити?
...
Якщо ви збережете `client secret` у коді вашого React або Angular додатку, 
...
### \#\# : Розбір конфігурації: Конвертер Ролей
...
* **Проблема:** Keycloak зберігає глобальні ролі користувача у вкладеній структурі JWT: 
* `realm_access.roles`. Spring Security за замовчуванням не знає, як їх звідти дістати.
* **Роль цієї конфігурації:** Цей бін виступає в ролі **"перекладача"**, 
* який "навчає" Spring Security, де саме всередині токену шукати ролі та як їх правильно 
* інтерпретувати (додаючи префікс `ROLE_`).

---

### \#\#Розбір конфігурації: Що таке CORS?

**CORS (Cross-Origin Resource Sharing)** — це механізм безпеки, вбудований у браузери, 
який за замовчуванням **блокує** HTTP-запити між різними "джерелами" (origins).

* **Origin (джерело)** = `схема (http) + хост (localhost) + порт (3000)`.
* Запит з фронтенду на `http://localhost:3000` до бекенду на `http://localhost:8080` — це **cross-origin запит**, 
* і він буде заблокований.
---

Якщо хоча б одна частина не збігається, браузер вважає запит "cross-origin".

1. Попередній запит (OPTIONS): 
...
"Так, я дозволяю запити з http://localhost:63342. І так, я дозволяю заголовок Authorization."
